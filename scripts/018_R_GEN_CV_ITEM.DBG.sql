CREATE OR REPLACE PACKAGE RIS.R_GEN_CV_ITEM_DBG IS
PROCEDURE SP_GEN_CEH_DBG (pin_from NUMBER, pin_to NUMBER);
PROCEDURE SP_GEN_CJH_DBG (pin_from NUMBER, pin_to NUMBER);
END;
/

CREATE OR REPLACE PACKAGE BODY RIS.R_GEN_CV_ITEM_DBG IS
PROCEDURE SP_GEN_CEH_DBG (pin_from NUMBER, pin_to NUMBER) IS
    pin 			NUMBER;
    tofi 			NUMBER;
    id_cv 			NUMBER;
    age 			NUMBER;
...
BEGIN
    ...
    BEGIN
        ...
        --DBMS_OUTPUT.PUT_LINE(wrcv_rec.PIN || ' DOB: ' || wrcv_rec.DOB || ' and CV ' || wrcv_rec.ID_CV
        --|| ' TOFI ' || wrcv_rec.ID_TOFI);
		...
		SELECT MINI, MAXI INTO age_min, age_max
        FROM GSD.G_RULES rul JOIN GSD.G_RULE_TYPES rult ON rul.ID_RULT=rult.ID_RULT
        WHERE NAME LIKE 'LVL%' AND SUBSTR(NAME,5) = TO_CHAR(itr_tmp);
        --DBMS_OUTPUT.PUT_LINE('AGE MIN :' || age_min || '; AGE_MAX:' || age_max);
		IF age > age_min THEN
        
        -- Select duration for title
        SELECT MINI INTO dur
        FROM GSD.G_RULES rul JOIN GSD.G_RULE_TYPES rult ON rul.ID_RULT=rult.ID_RULT
        WHERE NAME LIKE 'DUR_LVL%' AND SUBSTR(NAME,9) = TO_CHAR(itr_tmp);
        --DBMS_OUTPUT.PUT_LINE('DURATION :' || dur);
        
		-- Generate End and Graduate Date
        IF start_year_lst IS NULL
        THEN
            SELECT EXTRACT(YEAR FROM wrcv_rec.DOB)+ROUND(dbms_random.value(age_min, age_max)) 
            INTO grad_year FROM dual;
            --DBMS_OUTPUT.PUT_LINE('GEN_GRAD_YEAR: ' || grad_year);
        ELSE
            grad_year := start_year_lst;
        END IF;
        
        SELECT ROUND(dbms_random.value(4, 6)) INTO int_rnd_month FROM dual;
        SELECT ROUND(dbms_random.value(1, 28)) INTO int_rnd_day FROM dual;

		IF (int_rnd_month < 10)
            THEN
                SELECT TO_CHAR('0'||int_rnd_month) INTO grad_month FROM dual;
				SELECT TO_CHAR('0'||int_rnd_month-1) INTO end_month FROM dual;
            ELSE
        ...
        IF (int_rnd_day < 10)
            THEN
                SELECT TO_CHAR('0'||int_rnd_day) INTO grad_day FROM dual;
				SELECT TO_CHAR(int_rnd_day+ROUND(dbms_random.value(9,19),0)) INTO end_day FROM dual;
            ELSE
                grad_day := TO_CHAR(int_rnd_day);
				end_day := TO_CHAR(int_rnd_day-ROUND(dbms_random.value(1,9),0));
        END IF;
        
        SELECT TO_DATE(grad_day || '.'|| grad_month ||'.'|| grad_year, 'DD.MM.YYYY') INTO grad_date FROM dual;
		SELECT TO_DATE(end_day  || '.'|| end_month ||'.'|| end_year, 'DD.MM.YYYY') INTO end_date FROM dual;
        
        -- Generate Start Date
        SELECT grad_year-dur INTO start_year FROM dual;
        ...
        (
        SELECT eo.EIN, eof.ID_EFT, ROW_NUMBER() OVER (ORDER BY eo.EIN) RWN FROM
            GSD.G_EDU_TYPES eo
            ...
            WHERE eot.MAX_LVL = itr_tmp AND eof.ID_EFT=eft_id
        ...
        /*
        DBMS_OUTPUT.PUT_LINE('School:');
        DBMS_OUTPUT.PUT_LINE('RND MIN: ' || int_rnd_min || '; RND_MAX: ' || int_rnd_max);
        DBMS_OUTPUT.PUT_LINE('RND POSITION: ' || int_rnd_posit);*/
        
        SELECT EIN, ID_EFT INTO item_eo, item_eft  FROM
        (
        SELECT eo.EIN, eof.ID_EFT, ROW_NUMBER() OVER (ORDER BY eo.EIN) RWN FROM
            GSD.G_EDU_TYPES eo
            JOIN GSD.G_EDU_ORG_TYPE eot ON eo.ID_EOT=eot.ID_EOT
            ...
            AND (TO_CHAR(reo.DOE, 'YYYY_MM_DD') <= grad_day OR reo.DOE IS NULL)
            ORDER BY eo.EIN
        ) WHERE RWN = int_rnd_posit ;
        
        --SELECT NAME INTO skola FROM RIS.R_EDU_ORG WHERE EIN=item_eo;
        --SELECT NAME into item_eft FROM GSD.G_EDU_FIELD_TYPE WHERE ID_EFT=eft_tmp;
        ...
        SELECT MIN(RWN), MAX(RWN) INTO int_rnd_min, int_rnd_max
            FROM (
                SELECT VALUE2, ROW_NUMBER() OVER (ORDER BY rul.ID_RUL) RWN
                FROM
                    GSD.G_RULES rul
		...
		IF int_rnd_min IS NOT NULL AND int_rnd_max IS NOT NULL THEN
        SELECT round(dbms_random.value(int_rnd_min, int_rnd_max), 0) into int_rnd_posit FROM dual;

        SELECT VALUE2 INTO title FROM (
            SELECT VALUE2, ROW_NUMBER() OVER (ORDER BY rul.ID_RUL) RWN
                FROM
                    GSD.G_RULES rul
                    JOIN GSD.G_RULE_TYPES rult ON rul.ID_RULT=rult.ID_RULT
		...
		-- Select ID Title
		SELECT ID_TIT INTO item_tit FROM GSD.G_TITLE WHERE NAME = title;

		-- Select ID of Field
		SELECT MIN(RWN), MAX(RWN) INTO int_rnd_min, int_rnd_max
			FROM (
				SELECT eft.NAME, ROW_NUMBER() OVER (ORDER BY eof.ID_EFT) RWN
				FROM
        ...
        END LOOP;*/
         --DBMS_OUTPUT.PUT_LINE('TITLE: ' || title_last || ' LVL ' || lvl || ' SCHOOL -- ' ||item_eo || ' --- ' || skola);
         title_last := title;
         END IF;
        END IF;
        /*DBMS_OUTPUT.PUT_LINE('TITLE: ' || title_last || ' LVL ' || lvl || ' SCHOOL -- ' ||item_eo || ' --- ' || skola);
		DBMS_OUTPUT.PUT_LINE('EFT : ' || item_eft);
        DBMS_OUTPUT.PUT_LINE('END_DATE: ' || end_date);
        DBMS_OUTPUT.PUT_LINE('GRAD_DATE: ' || grad_date);
        DBMS_OUTPUT.PUT_LINE('START DATE: ' || start_date);*/
		itr_tmp := itr_tmp - 1;
        
        /*-- Check if end and graduation date are newer than date today
        IF (grad_date > sysdate) OR (end_date > sysdate)
        THEN
            end_date := NULL;
            grad_date := NULL;
        END IF;
    ...
    END LOOP;
END;
PROCEDURE SP_GEN_CJH_DBG (pin_from NUMBER, pin_to NUMBER) IS
    pin 			NUMBER;
    tofi 			NUMBER;
    id_cv 			NUMBER;
    age 			NUMBER;
...
BEGIN
    ...
    SAVEPOINT start_gen;
        ...
        FROM 
            GSD.G_RULES rul JOIN GSD.G_RULE_TYPES rult ON rul.ID_RULT=rult.ID_RULT
        WHERE 
            rult.NAME like 'WH%' 
        ...
        BEGIN
		    ...
		    IF age > age_min THEN
                ...
                IF start_year_lst IS NULL
                THEN
                    SELECT EXTRACT(YEAR FROM wrcv_rec.DOB)+ROUND(dbms_random.value(age_min+2, age_max)) 
                        INTO end_year FROM dual;
                ELSE
                ...
                END IF;
                    ...
                    THEN
    		          ...
    		          IF (int_rnd_month < 10)
                        ...
                        SELECT TO_CHAR(LAST_DAY(TO_DATE('01.'|| end_month || '.' || end_year ||'', 'DD.MM.YYYY')), 'DD') INTO end_day FROM dual;  
                        --end_day := LAST_DAY('01.'|| end_month || '.' || end_year ||'', 'DD.MM.YYYY');
                    ...
                    END IF;            
                      -- Generate Start Date
                        SELECT EXTRACT(YEAR FROM wrcv_rec.DOB)+ROUND(dbms_random.value(age_min, age_max-3)) 
                            INTO start_year FROM dual;
        ...
        SELECT round(dbms_random.value(int_rnd_min, int_rnd_max), 0) into int_rnd_posit FROM dual;
              ...
              SELECT ID_JI, ID_JP, CIN INTO item_ji, item_jp, item_cin
                FROM
                (
                SELECT
                    jp.ID_JP
                ...
                DBMS_OUTPUT.PUT_LINE('END_DATE: ' || TO_CHAR(end_date, 'YYYY.MM.DD'));
                DBMS_OUTPUT.PUT_LINE('START DATE: ' || TO_CHAR(start_date, 'YYYY.MM.DD'));
                DBMS_OUTPUT.PUT_LINE('LENGTH: ' || dur);
                DBMS_OUTPUT.PUT_LINE('INDUSTRY: ' || item_ji);
                DBMS_OUTPUT.PUT_LINE('POSITION: ' || item_jp);
                DBMS_OUTPUT.PUT_LINE('CIN: ' || item_cin || ' COMPANY: ' || company);
