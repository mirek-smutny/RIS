...
BEGIN
    ...
    LOOP
		...
		SELECT
			...
			WHERE
				--RWN = int_rnd_posit
				RWN = (SELECT round(dbms_random.value(int_rnd_min, int_rnd_max),0) FROM dual)
			ORDER BY
				NAME, SURNAME
			;
        ...
        SELECT
				ROUND(dbms_random.value(1970, 2000)),
				ROUND(dbms_random.value(1, 12)),
				ROUND(dbms_random.value(1, 28)) INTO int_rnd_year, int_rnd_month, int_rnd_day
			FROM
				dual;
        ...
        SELECT
			...
			WHERE
				rwn = 	(
							SELECT
									round(dbms_random.value(MIN(ID_TOFI), MAX(ID_TOFI)), 0)
								FROM
									GSD.G_TOFI
						);
