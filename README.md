...
These supportive packages were not part of the theme of the DT so it was omitted from the actual DT.
	...
	Requirements:
		- Oracle Database 19c
			- Partitioning (can be removed from tables)
	...
	Install:
		...
		- run changes/issue_2/install.sql - minimizes logging and optimized worker generation
	...
	Details:
		...
		- G_TOFI is table with different logical combinations of degrees, education organization, field type and industry positions. It is used to generate different employees profiles
			Example:
				...
				- person studies as car mechanic, will be employed only at positions at repair shop at company with relevant industrial assignment.
...
# RIS - ETL
	Added features to start ETL process

	Install:
		- Create tablespaces RIS_DWH_DATA and RIS_DWH_INDEX
		- run dwh_usr.sql
		- run install.sql

	Using:
		- Once you have data in original RIS
